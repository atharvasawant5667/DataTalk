...
def get_visualization_intent(question, columns):
    prompt = f"""
You are an expert data analyst and visualization assistant inside a conversational BI application.
...
- If unsure, make the safest reasonable choice

--------------------------------------------------
FORMAT 1: VISUALIZATION REQUESTS
--------------------------------------------------
If the intent is "visualization_requests", return JSON in EXACTLY this format:
...
Guidelines:
- Histogram → numeric column as x_column
- Bar/Pie → categorical column as x_column
- Line/Scatter → require both x_column and y_column
- If user mentions "top", "highest", "lowest", set n accordingly
- If unsure, choose bar chart

--------------------------------------------------
FORMAT 2: NON-VISUAL ANALYTICS
--------------------------------------------------
If the intent is NOT visualization_requests, return JSON in EXACTLY this format:

{{
  "category": "<one_of_the_6_categories>",
  "answer": "<clear, concise, human-readable answer>"
}}
    ...
    if section == "Data Upload & Cleaning":
        ...
        if user_question:
            ...
            try:
                ...
                st.subheader("Groq Interpretation (Tabular Format)")
                #st.table(table_df)
            except json.JSONDecodeError:
                st.error("Groq returned invalid JSON")
                st.stop()

            st.write("Groq response:", table_df)
...
# ---------------- TEXT / NUMERIC ANSWERS ----------------
            ...
            # BAR
                ...
                ax.set_title(f"Top 10 Categories in {x_col}")
                ax.set_xlabel(x_col)
                ax.set_ylabel("Count")
                ax.tick_params(axis="x", rotation=45)
